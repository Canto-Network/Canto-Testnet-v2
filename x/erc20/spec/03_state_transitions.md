...
A user registers a native Cosmos Coin. Once the proposal passes (i.e is Approvald by governance), the ERC20 module uses a factory pattern to deploy an ERC20 token contract representation of the Cosmos Coin. Note that the native canto coin cannot be registered, as any coin including "evm" in its denomination cannot be registered. Instead the canto token can be converted by Nomand's wrapped canto (Wcanto) contract.

1. User submits a `RegisterCoinProposal`
2. Validators of the canto Hub vote on the proposal using `MsgVote` and proposal passes
3. If Cosmos coin or IBC voucher exist on the bank module supply, create the [ERC20 token contract](https://github.com/canto/canto/blob/main/contracts/ERC20MinterBurnerDecimals.sol) on the EVM based on the ERC20Mintable ([ERC20Mintable by openzeppelin](https://github.com/OpenZeppelin/openzeppelin-contracts/tree/master/contracts/token/ERC20)) interface
    - Initial supply: 0
...
2. Validators of the canto chain vote on the proposal using `MsgVote` and proposal passes
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg canto, Atom,
